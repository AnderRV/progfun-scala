...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def timesRec(cList: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] = cList match {
      case Nil => acc
      case x :: xs => timesRec(xs.filter(_ != x), (x, cList.count(_ == x)) :: acc)
    }

    timesRec(chars.sorted, List())
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def rec(root: CodeTree, charList: List[Char], acc: List[Bit]): List[Bit] = charList match {
      case Nil => acc
      case x :: xs => root match {
        case Leaf(_, _) => rec(tree, xs, acc)
        case Fork(left, right, _, _) =>
          if (chars(left).contains(x)) rec(left, charList, acc :+ 0)
          else if (chars(right).contains(x)) rec(right, charList, acc :+ 1)
